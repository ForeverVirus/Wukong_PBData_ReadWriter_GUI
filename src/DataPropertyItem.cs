...
public class DataPropertyItem
{
    ...
    public string _PropertyDesc
    {
        get
        {
            ...
            {
                if (MainWindow.s_CustomDescriptionConfig.TryGetValue(
                        _DataItem._File._FileData.GetType().Name + "_" + _PropertyName, out var customDesc))
                    return customDesc;
                if (MainWindow.s_DefaultDescriptionConfig.TryGetValue(
                        _DataItem._File._FileData.GetType().Name + "_" + _PropertyName, out var desc)) return desc;
    ...
    public PropertyInfo _PropertyInfo
    {
        set
        {
            _propertyInfo = value;
            DisplayName =
                ComparisonTableController.Instance.GetData($"{_propertyInfo.DeclaringType.Name}.{_propertyName}",
                    out var content)
                    ? content
                    : "";
        }
